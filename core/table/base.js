...
export class SheetBase {
    ...
    rebuildHashSheetByValueSheet(valueSheet) {
        ...
        const usedCellUids = []; // Track used cell UIDs
        const newHashSheet = Array.from({ length: rows }, (_, i) => Array.from({ length: cols }, (_, j) => {
            ...
            // If a cell with the same value exists, reuse it, but exclude cells already used
            ...
            usedCellUids.push(cell.uid); // Mark the newly created cell as used
            return cell.uid;
        }));
